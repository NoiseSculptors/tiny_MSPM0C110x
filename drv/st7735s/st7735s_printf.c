...
#include <stdarg.h>
#if ST7735S_PRINTF_OPTIMIZE_SIZE == 0U
#include <string.h>
#endif
...
#endif
#if ST7735S_PRINTF_OPTIMIZE_SIZE == 1U
static void st7735s_printf_render_character(uint8_t character);
#endif
...
#if ST7735S_PRINTF_OPTIMIZE_SIZE == 1U
    st7735s_printf_render_character(value);
#else
...
{
    uint8_t pixels[16];
    const uint8_t *glyph = st7735s_printf_glyph(character);
    uint16_t glyph_row;
    uint16_t pixel_column;
    uint16_t pixel_index;
    uint16_t color;

    st7735s_set_window(printf_origin_x + printf_column * 8U,
        printf_origin_y + printf_row * 8U, 8U, 8U);
    st7735s_ramwr_begin();

    for (glyph_row = 0U; glyph_row < 8U; ++glyph_row) {
        pixel_index = 0U;
        for (pixel_column = 0U; pixel_column < 8U; ++pixel_column) {
            color = (glyph[glyph_row] & (uint8_t)(1U << pixel_column))
                ? printf_foreground : printf_background;
            pixels[pixel_index++] = (uint8_t)(color >> 8);
            pixels[pixel_index++] = (uint8_t)color;
        }
        st7735s_ramwr_write(pixels, sizeof(pixels));
    }
